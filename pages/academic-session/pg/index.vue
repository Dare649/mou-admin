      ...
      <div class="bg-white">
        <div class="container p-l-5">
          <ol class="breadcrumb breadcrumb-alt">
            ...
            <li class="breadcrumb-item active">PG</li>
          </ol>
        </div>
      </div>
      <!-- END BREADCRUMBS -->
        ...
        <div class="card card-default">
          <div class="card-header">
            ...
            <div class="pull-right sm-pull-reset">
              <nuxt-link to="/academic-session/pg/form/new" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> &nbsp; <strong>Add New PG Session</strong></nuxt-link>
            </div>
          </div>
          <div class="card-body">
              ...
              <table class="table table-striped table-condensed" id="basicTable">
                ...
                <template v-if="sessions.length && !loading">
                  ...
                  <tr v-for="(session, index) in sessions" :key="index">
                    <td>{{ session.pg_session_name }}</td>
                    <td>{{ numberFormat(session.pg_reg_admin_fees) }}</td>
                    <td>Admin</td>
                    ...
                    <td>
                      <div class="btn-group">
                        <nuxt-link :to="`/academic-session/pg/form/${session.id}`" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="Edit Record"><i class="fa fa-pencil"></i></nuxt-link>
                        <nuxt-link :to="`/academic-session/pg/${session.id}`" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="View Record"><i class="fa fa-eye"></i></nuxt-link>
                      </div>
                    </td>
                  </tr>
                </template>
...
export default {
  ...
  methods: {
    getAcademicSessions(page) {
      this.$axios.get(`api/pg-sessions?page=${page}`).then(res => {
    ...
    updateStatus(session, index, action) {
      ...
      this.$swal({
        ...
        if (result.value) {
          this.$axios.get(`api/pg-sessions/status/${session.id}?status=${action}`).then(res => {
            if(res.data.status) {
              if(action === 1) {
