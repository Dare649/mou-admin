        ...
        <div class="bg-white">
            <div class="container p-l-5">
                <ol class="breadcrumb breadcrumb-alt">
                    ...
                    <li class="breadcrumb-item"><nuxt-link :to="`/academic-session/direct-entry/${id}`">DE Options</nuxt-link></li>
                    <li class="breadcrumb-item active">Late Fee Slip for Direct Entry Registration</li>
                </ol>
            </div>
        </div>
        <!-- END BREADCRUMBS -->
...
export default {
    ...
    methods: {
      downloadSampleCSV() {
        this.$axios.get('api/de-sessions/late-fees-slip/sample').then(res => {
          if (res.data.success) {
      ...
      getSlips(page) {
        this.$axios.get(`api/de-sessions/late-fees-slip?session_id=${this.id}&slug=${this.feeType}`).then(res => {
          this.loading = false
      ...
      deleteRecord(e, id) {
        ...
        this.$swal({
          ...
          if (result.value) {
            this.$axios.delete(`api/de-sessions/late-fees-slip/${id}`).then(res => {
              if (res.data.status) {
