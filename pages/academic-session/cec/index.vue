    ...
    <div class="content sm-gutter">
      ...
      <div class="bg-white">
        <div class="container p-l-5">
          <ol class="breadcrumb breadcrumb-alt">
            ...
            <li class="breadcrumb-item active">CEC</li>
          </ol>
        </div>
      </div>
      <!-- END BREADCRUMBS -->
      ...
      <div class="container sm-padding-10 p-t-20 p-l-0 p-r-0">
        <div class="card card-default">
          <div class="card-header">
            <h3 class="text-primary no-margin pull-left sm-pull-reset">CEC</h3>
                ...
                <tbody>
                  <template v-if="sessions.length && !loading">
                    <tr v-for="(session, index) in sessions" :key="index">
                      ...
                      <td>
                        <div class="btn-group">
                          <nuxt-link :to="`/academic-session/cec/form/${session.id}`" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="Edit Record"><i class="fa fa-pencil"></i></nuxt-link>
                          <nuxt-link :to="`/academic-session/cec/${session.id}`" class="btn btn-default btn-sm" data-toggle="tooltip" data-placement="top" title="View Record"><i class="fa fa-eye"></i></nuxt-link>
                        </div>
                      </td>
                    </tr>
                  </template>
...
export default {
  ...
  methods: {
    getAcademicSessions(page) {
      this.$axios.get(`api/cec-sessions?page=${page}`).then(res => {
    ...
    updateStatus(session, index, action) {
      ...
      this.$swal({
        ...
        if (result.value) {
          this.$axios.get(`api/cec-sessions/status/${session.id}?status=${action}`).then(res => {
            if(res.data.status) {
              if(action === 1) {
